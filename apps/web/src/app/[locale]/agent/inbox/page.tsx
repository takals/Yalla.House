...
export default async function AgentInboxPage() {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-[#0F1117] tracking-tight">{t('title')}</h1>
        <p className="text-sm text-[#656565] mt-2">{t('subtitle')}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E2E4EB] p-6 mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-[#0F1117] mb-1">{t('yourEmail')}</h2>
            <p className="text-base font-mono text-[#D4764E]">{yallaEmail}</p>
          </div>
          <div className="text-xs text-[#999] text-right max-w-xs">{t('emailExplainer')}</div>
        </div>
      </div>
      ...
      {threads.length > 0 ? (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  {thread.listing && (
                    <p className="text-sm text-[#656565]">{thread.listing.place_id}</p>
                  )}
                </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E2E4EB] p-12 text-center">
          ...
          <h3 className="text-base font-semibold text-[#0F1117] mb-1">{t('noMessages')}</h3>
          <p className="text-sm text-[#656565] max-w-sm mx-auto">{t('noMessagesDesc')}</p>
        </div>
      )}
    </div>
  )
