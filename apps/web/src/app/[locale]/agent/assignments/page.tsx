...
export default function AgentAssignmentsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Supabase returns relation joins as arrays even for many-to-one.
      // Normalize to single object for the UI.
      const normalized: Assignment[] = (data ?? []).map((row: Record<string, unknown>) => {
        const listingArr = (row.listing as Assignment['listing'][] | Assignment['listing'] | null) ?? null
        const ownerArr = (row.owner as Assignment['owner'][] | Assignment['owner'] | null) ?? null
        return {
          id: row.id as string,
          status: row.status as string,
          created_at: row.created_at as string,
          tier: row.tier as string,
          can_edit_listing: row.can_edit_listing as boolean,
          can_manage_viewings: row.can_manage_viewings as boolean,
          can_negotiate: row.can_negotiate as boolean,
          can_message_buyers: row.can_message_buyers as boolean,
          listing: Array.isArray(listingArr)
            ? (listingArr[0] ?? { id: '', title: '', address: '', price: 0 })
            : (listingArr ?? { id: '', title: '', address: '', price: 0 }),
          owner: Array.isArray(ownerArr)
            ? (ownerArr[0] ?? { id: '', name: '', email: '' })
            : (ownerArr ?? { id: '', name: '', email: '' }),
        }
      })
