...
export function ViewingCalendar({ listingId, authenticated, isOwner, locale, placeId, preselectedSlotId }: Props) {
  ...
  if (loading) {
    return (
      <div data-booking-slots className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-[#E2E4EB]/60">
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from(grouped.entries()).map(([day, daySlots]) => (
              <div key={day} className="bg-[#FAFBFC] rounded-xl p-4 border border-[#E2E4EB]/50">
                <p className="text-xs font-bold text-[#0F1117] mb-3 uppercase tracking-wide">
          ...
          {selectedSlot && authenticated && !isOwner && (
            <div className="bg-[#D4764E]/5 rounded-xl p-5 border border-[#D4764E]/15">
              <label className="text-xs font-semibold text-[#5E6278] block mb-1.5">
                {t('calendarAddNote')}
              </label>
              <textarea
                value={notes}
                ...
                rows={2}
                className="w-full border border-[#E2E4EB] rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#D4764E]/30 resize-none"
              />
              {error && <p className="text-xs text-red-600 mt-2">{error}</p>}
              <button
                onClick={handleBook}
                disabled={booking}
                className="mt-3 w-full bg-[#D4764E] hover:bg-[#BF6840] text-white font-bold text-sm py-3.5 rounded-xl transition-colors disabled:opacity-50 shadow-md shadow-[#D4764E]/20"
              >
      ...
      {!hasSlots && (
        <div className="relative">
          <div className="opacity-30 pointer-events-none select-none">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {mockDays.map(day => (
                <div key={day.day} className="bg-[#FAFBFC] rounded-xl p-4 border border-[#E2E4EB]/50">
                  <p className="text-xs font-bold text-[#0F1117] mb-3 uppercase tracking-wide">{day.day}</p>
                  <div className="flex flex-col gap-1.5">
                    {day.times.map(time => (
                      <div
                        key={time}
                        className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-semibold border border-[#E2E4EB] bg-white text-[#999]"
                      >
                        <Clock size={13} />
                        {time}
                      </div>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl px-8 py-6 shadow-xl border border-[#E2E4EB] text-center max-w-sm">
              {isOwner ? (
                <>
                  <p className="font-bold text-[#0F1117] mb-1.5">
                  ...
                  <a
                    href="/owner/viewings"
                    className="inline-flex items-center gap-2 bg-[#D4764E] hover:bg-[#BF6840] text-white font-bold text-sm px-5 py-2.5 rounded-xl transition-colors shadow-md shadow-[#D4764E]/20"
                  >
                    <Plus size={14} />
