    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-surface rounded-2xl border border-[#E2E4EB] p-12 text-center">
          ...
          <p className="text-[#5E6278] font-medium">{tx(t, 'noOffers')}</p>
        </div>
      ) : (
    ...
    <div className={`bg-surface rounded-2xl border border-[#E2E4EB] overflow-hidden ${isPending ? 'opacity-60' : ''}`}>
      ...
      <div className="px-5 py-4 border-b border-[#E2E4EB] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-[#0F1117] text-sm">
      ...
      <div className="px-5 py-4">
        ...
        {offer.finance_status && (
          <div className="mb-3">
            <p className="text-xs font-semibold text-[#5E6278] mb-1">{tx(t, 'finance')}</p>
        ...
        {offer.conditions && (
          <div className="mb-3">
            <p className="text-xs font-semibold text-[#5E6278] mb-1">{tx(t, 'conditions')}</p>
            <p className="text-sm text-[#0F1117]">{offer.conditions}</p>
          </div>
        )}

        {offer.message && (
          <div className="mb-3">
            <p className="text-xs font-semibold text-[#5E6278] mb-1">{tx(t, 'message')}</p>
            <p className="text-sm text-[#0F1117] bg-[#FAFBFC] rounded-lg p-3 border border-[#E2E4EB]">
              {offer.message}
            </p>
          </div>
        )}
        ...
        {offer.move_in_date && (
          <div className="mb-3">
            <p className="text-xs font-semibold text-[#5E6278] mb-1">{tx(t, 'moveInDate')}</p>
            <p className="text-sm text-[#0F1117] flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-[#5E6278]" />
      ...
      {/* Actions */}
      {isActionable && (
        <div className="px-5 py-3 border-t border-[#E2E4EB] bg-[#FAFBFC] flex gap-3">
          <button
...
function Detail({ icon: Icon, label, value, highlight }: {
  ...
  return (
    <div>
      <p className="text-xs font-semibold text-[#5E6278] mb-0.5 flex items-center gap-1">
        <Icon className="w-3 h-3" />
        {label}
      </p>
      <p className={`text-sm font-semibold ${highlight ? 'text-[#D4764E]' : 'text-[#0F1117]'}`}>
        {value}
      </p>
    </div>
  )
