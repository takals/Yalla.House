...
export function BriefSendForm({ listingId, agents, existingAssignmentAgentIds, translations }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface rounded-2xl border border-[#E2E4EB] p-6">
      <h2 className="text-lg font-bold text-[#0F1117] mb-6">{translations.sendBrief}</h2>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-[#0F1117]">{translations.selectAgents}</h3>
        ...
        {availableAgents.length === 0 ? (
          <div className="p-4 bg-[#F8F9FA] rounded-lg border border-[#E2E4EB] text-center">
            <p className="text-sm text-[#5E6278]">{translations.allAssigned}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {availableAgents.map(agent => (
              <label
                key={agent.id}
                className="flex items-start gap-3 p-3 rounded-lg border border-[#E2E4EB] hover:bg-[#F8F9FA] cursor-pointer transition-colors"
              >
                <input
                  type="checkbox"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#0F1117]">{agent.full_name}</p>
                  <p className="text-xs text-[#5E6278]">{agent.agency_name}</p>
                  {agent.verified && (
      ...
      {/* Tier selection */}
      <div className="mb-8">
        <h3 className="text-sm font-bold text-[#0F1117] mb-4">{translations.serviceTier}</h3>
            ...
            return (
              ...
              >
                <input
                  ...
                  className="sr-only"
                />
                <p className="text-sm font-bold text-[#0F1117] mb-1">{tierLabels[tier]}</p>
                <p className="text-xs text-[#5E6278]">{tierDescs[tier]}</p>
              </label>
            )
          })}
        </div>
      ...
      {/* Notes */}
      <div className="mb-8">
        <label className="block text-sm font-bold text-[#0F1117] mb-2">{translations.additionalNotes}</label>
        <textarea
          value={notes}
          onChange={e => setNotes(e.target.value)}
          placeholder={translations.notesPlaceholder}
          className="w-full p-3 rounded-lg border border-[#E2E4EB] focus:outline-none focus:ring-2 focus:ring-brand/50 text-sm"
          rows={4}
        />
      </div>
      ...
      <button
        ...
        className={`w-full py-3 rounded-lg font-semibold text-[#0F1117] transition-all ${
