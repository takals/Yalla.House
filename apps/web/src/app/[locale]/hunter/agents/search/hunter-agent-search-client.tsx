...
import {
  Search, MapPin, Shield, Phone, Mail, Globe,
...
export function HunterAgentSearchClient({ translations: t }: Props) {
  ...
  const [connectResult, setConnectResult] = useState<{ success: number; failed: number } | null>(null)
  ...
  const handleConnect = async () => {
    ...
    try {
      const res = await fetch('/api/agents/connect', {
        ...
        body: JSON.stringify({ agentProfileIds: Array.from(selectedAgents) }),
      })
      const data = await res.json()
      setConnectResult({ success: data.connected ?? 0, failed: data.failed ?? 0 })
      setSelectedAgents(new Set())
    } catch {
      setConnectResult({ success: 0, failed: selectedAgents.size })
    } finally {
      setConnecting(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      {connectResult && (
        <div className={`rounded-xl border p-4 text-sm font-semibold ${
          ...
          {connectResult.success > 0
            ? `${connectResult.success} agent${connectResult.success === 1 ? '' : 's'} connected successfully!`
            : 'Failed to connect agents. Please try again.'}
        </div>
      )}
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border-default p-4 shadow-lg z-50">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            ...
            <button
              ...
              disabled={connecting}
              className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand text-white font-bold rounded-xl hover:bg-brand-hover transition-colors text-sm disabled:opacity-50"
            >
              {connecting ? <Loader2 size={16} className="animate-spin" /> : <ArrowRight size={16} />}
              {t.connectButton ?? 'Connect with Selected'}
            </button>
          </div>
        </div>
      )}
